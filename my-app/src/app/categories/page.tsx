    ...
    <div className="min-h-screen stoneBg text-[var(--foreground)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {categories.map((c) => (
            <Link key={c.slug} href={`/products?category=${c.name}`} className="group overflow-hidden border border-[#cfc9c0] bg-white">
              ...
              <div className="p-5">
                ...
                <span className="inline-block mt-3 bg-[var(--gold)] text-black px-3 py-1 font-semibold hover:bg-white hover:text-[var(--leaf)]">{t('category.All')}</span>
              </div>
            </Link>
          ))}
        </div>
