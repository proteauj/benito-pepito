...
function slugify(input: string) {
  return input.toLowerCase().replace(/&/g, 'and').replace(/\s+/g, '-');
}
...
export default function CategoryPage() {
  ...
  if (error || !categoryTitle) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-xl text-red-500">Category not found</p>
          <Link href="/" className="mt-4 inline-block bg-emerald-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-emerald-700">
            Back to Home
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <label className="flex items-center space-x-3 p-3 rounded">
            <input type="checkbox" checked={onlyAvailable} onChange={e => setOnlyAvailable(e.target.checked)} />
            <span className="text-[var(--leaf)] font-medium">Only available</span>
          </label>
          <select
            className="p-3 rounded bg-[var(--background)] text-black border border-[color-mix(in_oklab,var(--leaf)_25%,transparent)] focus:outline-none focus:ring-2 focus:ring-[var(--leaf)]/40"
            value={artistFilter}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredSorted.map((p) => (
            <div key={p.id} className="group bg-white rounded-xl overflow-hidden border border-[#cfc9c0]">
              ...
              <div className="p-5 text-black">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-baseline space-x-2">
                    ...
                    {p.originalPrice && (
                      <span className="text-sm line-through text-gray-500">${p.originalPrice}</span>
                    )}
                  </div>
                  <div className="flex space-x-2">
                    <Link href={`/product/${p.slug}`} className="px-3 py-2 rounded-lg bg-[var(--gold)] text-black text-sm font-semibold hover:bg-[var(--gold-dark)]">View</Link>
                    <button
                      onClick={() => addToCart(p, 1)}
                      disabled={!p.inStock}
                      className="px-3 py-2 rounded-lg bg-[var(--gold)] text-black text-sm font-semibold hover:bg-[var(--gold-dark)] disabled:bg-gray-400 disabled:cursor-not-allowed"
                    >
                      {p.inStock ? 'Add' : 'Sold'}
                    </button>
                  </div>
